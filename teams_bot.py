...
class TeamsBot(ActivityHandler):
    ...
    async def get_langflow_response(self, message: str) -> str:
        ...
        try:
            ...
            if LANGFLOW_FLOW_ID:
                response = await self._call_langflow(message)
                return response

            # Otherwise, use a simple echo response
            return f"You said: {message}\n\n(Configure LANGFLOW_URL and LANGFLOW_FLOW_ID in .env to enable AI responses)"

        except Exception as e:
            print(f"Error getting Langflow response: {e}")
            return "Sorry, I encountered an error processing your message. Please try again."

    async def _call_langflow(self, message: str) -> str:
        """
        ...
        try:
            ...
            flow_parts = LANGFLOW_FLOW_ID.split("/")
            flow_id = flow_parts[0] if flow_parts else LANGFLOW_FLOW_ID
